...
export function gerarDeck(data: DadosApres, meses: string[], secoes: Set<Secao>): string {
  ...
  const css = `${baseCss()}
    ...
    .slide{position:absolute;inset:0;width:1280px;height:720px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:52px 78px;opacity:0;visibility:hidden;overflow:hidden;transition:opacity .35s ease;text-align:center}
...
function exportarPDF(){var B=document.querySelectorAll('.expbtn,.expbtn-big');function T(t){for(var j=0;j<B.length;j++)B[j].textContent=t}T('Gerando PDF...');function L(u){return new Promise(function(r,e){var s=document.createElement('script');s.src=u;s.onload=r;s.onerror=e;document.head.appendChild(s)})}var P=Promise.resolve();if(!window.html2canvas)P=P.then(function(){return L('https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js')});if(!(window.jspdf&&window.jspdf.jsPDF))P=P.then(function(){return L('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js')});P.then(function(){var sl=[].slice.call(document.querySelectorAll('.slide')),st=document.getElementById('stage');var pt=st.style.transform,ptop=st.style.top,pl=st.style.left,ai=0;for(var z=0;z<sl.length;z++){if(sl[z].classList.contains('active'))ai=z}st.style.transform='none';st.style.top='0';st.style.left='0';var bigs=document.querySelectorAll('.expbtn-big');for(var q=0;q<bigs.length;q++)bigs[q].style.display='none';var bg=getComputedStyle(document.body).backgroundColor||'#0A0A0A';var pdf=new window.jspdf.jsPDF({orientation:'landscape',unit:'px',format:[1280,720]});var k=0;function done(){for(var z=0;z<sl.length;z++)sl[z].classList.toggle('active',z===ai);st.style.transform=pt;st.style.top=ptop;st.style.left=pl;for(var q=0;q<bigs.length;q++)bigs[q].style.display='';pdf.save('apresentacao.pdf');for(var y=0;y<B.length;y++)B[y].textContent=B[y].className.indexOf('big')>-1?'⬇ Baixar em PDF':'⬇ Baixar PDF'}function nx(){if(k>=sl.length){done();return}for(var z=0;z<sl.length;z++)sl[z].classList.toggle('active',z===k);window.html2canvas(sl[k],{width:1280,height:720,scale:2,backgroundColor:bg,logging:false}).then(function(cv){var im=cv.toDataURL('image/jpeg',0.92);if(k>0)pdf.addPage([1280,720],'landscape');pdf.addImage(im,'JPEG',0,0,1280,720);k++;nx()}).catch(function(){k++;nx()})}nx()}).catch(function(){T('Falhou (precisa de internet)')})}
